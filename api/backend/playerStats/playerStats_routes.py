...
# Assuming you have a db_connection module similar to the one in your example
...
def get_game_instance_id(cursor, profileID, gameID):
    """Retrieves the gameInstanceID using the provided cursor."""
    cursor.execute(
...
def get_stat_table_id(cursor, profileID, gameID):
    """Retrieves the statTableID for a player in a game using the provided cursor."""
...
# --- 1. PLAYER SUMMARY STATS ENDPOINTS ---

@playerStats.route('/playerstats/summary/<int:profileID>/<int:gameID>', methods=['GET'])
def get_player_summary_stats(profileID, gameID):
    """GET player's overall stats for a specific game."""
    current_app.logger.info(f'GET /playerstats/summary/{profileID}/{gameID}')
...
@playerStats.route('/playerstats/summary/<int:profileID>/<int:gameID>', methods=['POST'])
def create_player_summary_stats(profileID, gameID):
    """POST a new player stats summary."""
    current_app.logger.info(f'POST /playerstats/summary/{profileID}/{gameID}')
    cursor = db.get_db().cursor()
    game_instance_id = get_game_instance_id(cursor, profileID, gameID)
    if not game_instance_id:
    ...
    params = (
        game_instance_id,
        stats_info.get('kills', 0), stats_info.get('deaths', 0), stats_info.get('assists', 0),
        stats_info.get('totalDamage', 0), stats_info.get('totalHeadshots', 0),
        stats_info.get('totalShotsHit', 0), stats_info.get('totalWins', 0)
    )
    cursor.execute(query, params)
    db.get_db().commit()
    return make_response(jsonify({'message': 'Player summary stats created'}), 201)

@playerStats.route('/playerstats/summary/<int:profileID>/<int:gameID>', methods=['PUT'])
def update_player_summary_stats(profileID, gameID):
    """PUT (update) a player's stats summary."""
    current_app.logger.info(f'PUT /playerstats/summary/{profileID}/{gameID}')
    cursor = db.get_db().cursor()
    game_instance_id = get_game_instance_id(cursor, profileID, gameID)
    if not game_instance_id:
        return make_response(jsonify({'error': 'Player not found for this game'}), 404)
    ...
    stats_info = request.json
    update_fields = [f"{key} = %s" for key in stats_info if key in ['kills', 'deaths', 'assists', 'totalDamage', 'totalHeadshots', 'totalShotsHit', 'totalWins']]
    ...
    db.get_db().commit()
    return make_response(jsonify({'message': 'Player summary stats updated'}), 200)

@playerStats.route('/playerstats/summary/<int:profileID>/<int:gameID>', methods=['DELETE'])
def delete_player_summary_stats(profileID, gameID):
    """DELETE a player's stats summary."""
    current_app.logger.info(f'DELETE /playerstats/summary/{profileID}/{gameID}')
    cursor = db.get_db().cursor()
    game_instance_id = get_game_instance_id(cursor, profileID, gameID)
    if not game_instance_id:
        return make_response(jsonify({'error': 'Player not found for this game'}), 404)
    ...
    db.get_db().commit()
    return make_response(jsonify({'message': 'Player summary stats deleted'}), 200)
...
@playerStats.route('/playerstats/weapon/<int:profileID>/<int:gameID>', methods=['GET'])
def get_all_player_weapon_stats(profileID, gameID):
    """GET all weapon stats for a player in a game."""
...
@playerStats.route('/playerstats/weapon/<int:profileID>/<int:gameID>', methods=['POST'])
...
@playerStats.route('/playerstats/weapon/<int:profileID>/<int:gameID>/<int:weaponID>', methods=['PUT'])
...
@playerStats.route('/playerstats/weapon/<int:profileID>/<int:gameID>/<int:weaponID>', methods=['DELETE'])
...
@playerStats.route('/playerstats/map/<int:profileID>/<int:gameID>/<int:mapID>', methods=['GET'])
...
@playerStats.route('/playerstats/map/<int:profileID>/<int:gameID>', methods=['GET'])
def get_all_player_map_stats(profileID, gameID):
    """GET all map stats for a player in a game."""
...
@playerStats.route('/playerstats/map/<int:profileID>/<int:gameID>', methods=['POST'])
...
@playerStats.route('/playerstats/map/<int:profileID>/<int:gameID>/<int:mapID>', methods=['PUT'])
...
@playerStats.route('/playerstats/map/<int:profileID>/<int:gameID>/<int:mapID>', methods=['DELETE'])
